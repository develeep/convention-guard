...
- **주석과 문자열은 코드가 아닙니다 (3.0).** 주석 처리해 둔 `console.log(` 나 문자열 안의 코드 조각은 지적하지 않습니다. 규칙마다 `not_in`·`in_scope`·`block_empty` 로 켭니다.

Python 3.9+ 외 의존성 없음 (PyYAML 이 있으면 사용). Laravel / PHP, Next / React / Nest / JS·TS, Go 규칙 36개 번들.

1.x 에서 올라오신다면 **[1.x → 3.0 마이그레이션](docs/migration-3.0.md)** 을 먼저 보세요 — Python 최소 버전이 3.9 로 올라갔고, `migrate.py` 가 규칙에 구조 조건을 넣어 줍니다.
