...
| 레거시 규모, 도입 전 감사 | `--all` (결과가 많음 — 요청받았을 때만) |
...
종료 코드 2 는 "검사 불가"입니다(git 아님, 잘못된 범위, 설정 오류). 결과 없음으로 읽지 말고 stderr 를 전하세요. 옵션 전체와 CI 연동: [references/cli.md](references/cli.md)
